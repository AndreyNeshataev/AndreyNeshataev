...
class Profile(models.Model):
    ...
    def get_status(self):
        total_amount = cache.get_or_set('total_amount',
                                        PurchaseHistory.objects.filter(user_id=self.user.id).aggregate(Sum('total_price')),
                                        30)
        if 5000 <= total_amount['total_price__sum'] < 50000:
            self.status = 'b'
        elif total_amount['total_price__sum'] >= 50000:
            self.status = 'c'
        else:
            self.status = 'a'
        self.save()
